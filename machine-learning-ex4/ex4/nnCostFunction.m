...
% Initialize X input to include a row of ones
...
a1 = X;
z2 = Theta1 * a1';
a2 = [ones(1, size(z2, 2)); sigmoid(z2)]; %include a row of ones (a_0^(2))
z3 = Theta2 * a2;
a3 = sigmoid(z3);
h = a3;
...
% y is now a 10x5000 matrix instead of a 5000x1 vector

J = 1/m * sum(sum(-yMatrix.*log(h) - (1-yMatrix).*log(1-h)));

%regularized cost function

% First, we get rid of the terms that correspond to the bias (i.e. the first columns) in Thetas
